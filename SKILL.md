...
- Browser-session HTTP: after a real browser profile has passed a site challenge, `fetch(url, source="session")` fetches same-domain pages with that browser's user agent and matching cookies via the authority pipeline; pair it with the domain's parser or embedded-data extractor.
- Robust solved sessions: `fetch(url, source="session")` reuses the browser's solved challenge state. The authority pipeline manages session seeding and cookie attachment automatically.
...
- Structured fetching: `fetch(url)` returns a `Response` with `.css(selector)`, `.css_text(selector)`, `.xpath(expr)`, `.text`, `.html`, `.next_data()` (Next.js JSON), `.json_ld(type)` (schema.org blocks), `.embedded_json(var)` (window.VAR assignments). Use `source="http"` for static pages, `source="session"` for browser-session cookies, `source="browser"` for JS-required pages, `source="auto"` (default) to route through the AccessPlane authority pipeline. The authority cascade tries HTTP, then browser session, then full browser render; challenges produce handoff IDs rather than automated bypass.
- Cloudflare Turnstile: when `wait_for_content()` reports a blocked page on a Cloudflare site, the authority pipeline produces a challenge handoff ID. See `interaction-skills/waf-bypass.md` for diagnostic and recovery guidance.
...
- Authority pipeline: `fetch(source="auto")` routes through the AccessPlane authority pipeline by default. PolicyEngine governs all actions through risk levels (R0-R5). Agent runtime uses subprocess sandbox with audit hooks (`sys.addaudithook`).
