...
def main() -> int:
    ...
    ax.text(9, 6200, "8B models", ha="center", fontsize=10, color="#B0B0B0", style="italic", fontweight="bold")
    ax.text(30, 6200, "36B models", ha="center", fontsize=10, color="#B0B0B0", style="italic", fontweight="bold")
    ax.text(90, 6200, "200B+ models", ha="center", fontsize=10, color="#B0B0B0", style="italic", fontweight="bold")
    ...
    for name, mem, cost, toks, cat in systems:
        ...
        ax.scatter(mem, cost, s=size, c=color, alpha=0.9, edgecolors="white", linewidth=2.5, zorder=5)
        ax.text(mem, cost, f"{toks}", ha="center", va="center", fontsize=9, color="white", fontweight="bold", zorder=7)
        ...
        bbox_props = None
        if is_primary_strix:
            bbox_props = dict(boxstyle="round,pad=0.4", facecolor="#FEF2F2", edgecolor="#DC2626", alpha=0.95, linewidth=1.5)
        ...
        ax.annotate(
            ...
            color=label_color,
            arrowprops=dict(arrowstyle="->", color="#B0B0B0", lw=1, connectionstyle="arc3,rad=0.12"),
            zorder=6,
            bbox=bbox_props,
        )
    ...
    ax.annotate(
        ...
        color="#991B1B",
        ha="center",
        arrowprops=dict(arrowstyle="-|>", color="#DC2626", lw=2.5, connectionstyle="arc3,rad=-0.2", mutation_scale=15),
        zorder=8,
        bbox=dict(boxstyle="round,pad=0.6", facecolor="#FEF2F2", edgecolor="#DC2626", linewidth=2, alpha=0.95),
    )
    ...
    ax.set_xlabel("Memory (GB)", fontsize=13, fontweight="bold", labelpad=12, color="#1F2937")
    ax.set_ylabel("Total System Cost (AU$)", fontsize=13, fontweight="bold", labelpad=12, color="#1F2937")
    ax.set_title("What Can You Actually Run?\nMemory vs Cost - Local LLM Hardware in Australia", fontsize=18, fontweight="bold", pad=22, color="#111827")
    ax.set_xlim(0, 140)
    ...
    leg = ax.legend(handles=legend_items, loc="upper left", fontsize=9, framealpha=0.95, edgecolor="#D1D5DB", fancybox=True)
