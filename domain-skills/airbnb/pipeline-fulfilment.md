...
| Reviews / rating distribution | High for public-visible fields | 27 own-listing review rows; listing-level review count captured for all 27; overall rating captured where Airbnb exposes listing average; star distribution/category fields captured where Airbnb exposes them or visible individual review stars support the estimate. `rating_display_state`, `star_distribution_source`, and `rating_category_source` distinguish absent Airbnb widgets from parser failure. Host aggregate reviews are explicitly excluded from listing-level review count/rating. |
