...
| Reviews / rating distribution | High for public-visible fields | 27 own-listing review summary rows; listing-level review count captured for all 27; overall rating captured where Airbnb exposes listing average; star distribution/category fields captured where Airbnb exposes them or visible individual review stars support the estimate. `rating_display_state`, `star_distribution_source`, `star_distribution_confidence`, `visible_individual_review_star_count`, and `rating_category_source` distinguish absent or partial Airbnb widgets from parser failure. The collector also emits `airbnb_own_public_review_snapshot` rows for visible individual public reviews. Host aggregate reviews are explicitly excluded from listing-level review count/rating. |
