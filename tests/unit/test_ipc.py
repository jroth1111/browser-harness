...
def test_read_port_file_rejects_malformed_token(tmp_path, monkeypatch):
    monkeypatch.setattr(ipc, "_TMP", tmp_path)
    ipc.port_path("default").write_text('{"port": 9222, "token": 12345}', encoding="utf-8")

    assert ipc._read_port_file("default") == (None, None)


def test_read_port_file_rejects_out_of_range_port(tmp_path, monkeypatch):
    monkeypatch.setattr(ipc, "_TMP", tmp_path)
    ipc.port_path("default").write_text('{"port": 70000, "token": "tok"}', encoding="utf-8")
