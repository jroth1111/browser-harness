...
def test_sb_inject_cookies_ignores_malformed_cookie_file(tmp_path, monkeypatch):
    module = load_module("sb_helpers")
    ...
    added = []
    sb = SimpleNamespace(driver=SimpleNamespace(add_cookie=lambda cookie: added.append(cookie)))

    assert module.inject_cookies(sb, "doordash") == 0
    assert added == []
